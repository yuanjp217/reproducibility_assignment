...
Subjects were shown a stimulus array consisting of a target stimulus (a letter) and three distracting numbers. All stimuli were randomly assigned a different color (red, blue, yellow, or magenta). The stimulus array was then masked using "@" symbols, and then the screen turned blank (dark grey). Subjects then either saw a *Presurprise* trial or a *Surprise* trial. 

The *Presurprise* trial consisted of a location task, where the screen showed the numbers 1, 2, 3, and 4, located at the positions of the original 4 stimuli. Subjects were asked to indicate which position the letter stimulus was located at.

In the *Surprise* trial, the subjects were asked to identify the critical attribute of the stimulus (the identity of the letter) and also a task-irrelevant attribute of the target (the letter's color). 
...
reportObject <- reproCheck(reportedValue = '0.3', obtainedValue = color/20, valueType = 'other', round = TRUE)
...
reportObject <- reproCheck(reportedValue = '0.25', obtainedValue = identity/20, valueType = 'other', round = TRUE)
...
reportObject <- reproCheck(reportedValue = '0.8', obtainedValue = location/20, valueType = 'other', round = TRUE)
```
...
reportObject <- reproCheck(reportedValue = '0.7', obtainedValue = ctrl1_color, valueType = 'other', round = TRUE)
#identity task
ctrl1_identity <- sum(ctrl$acc_identity_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.75', obtainedValue = ctrl1_identity, valueType = 'other', round = TRUE)
#location task
ctrl1_location <- sum(ctrl$acc_location_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.8', obtainedValue = ctrl1_location, valueType = 'other', round = TRUE)
```
...
reportObject <- reproCheck(reportedValue = '0.75', obtainedValue = ctrl2_color, valueType = 'other', round = TRUE)
#identity task
ctrl2_identity <- sum(ctrl2$acc_identity_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.75', obtainedValue = ctrl2_identity, valueType = 'other', round = TRUE)
#location task
ctrl2_location <- sum(ctrl2$acc_location_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.85', obtainedValue = ctrl2_location, valueType = 'other', round = TRUE)
```
...
reportObject <- reproCheck(reportedValue = '0.7', obtainedValue = ctrl3_color, valueType = 'other', round = TRUE)
#identity task
ctrl3_identity <- sum(ctrl3$acc_identity_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.8', obtainedValue = ctrl3_identity, valueType = 'other', round = TRUE)
#location task
ctrl3_location <- sum(ctrl3$acc_location_report == 1) / 20
reportObject <- reproCheck(reportedValue = '0.8', obtainedValue = ctrl3_location, valueType = 'other', round = TRUE)
```
```{r 4th control}
ctrl4 = acc_d %>%
  filter(trial == 160)
#color task
sum(ctrl4$acc_color_report == 1) / 20

#identity task
sum(ctrl4$acc_identity_report == 1) / 20

#location task
sum(ctrl4$acc_location_report == 1) / 20
```

## Inferential statistics

```{r chi2}
#"The improvement in each case was significant—color: 70% versus 30%, χ2(1, N = 40) = 6.40, p = .011, ϕ = .40; identity: 75% versus 25%, χ2(1, N = 40) = 10.00, p < .005, ϕ = .50. "

#chi squared test of independence contrasting surprise and control (the trial right after surprise)
#color chi square; 70% vs. 30%
chisq.test(ctrl$acc_color_report,surprise_data$acc_color_report, correct = FALSE)

#identity chi square; 75% vs. 25%
chisq.test(ctrl$acc_identity_report,surprise_data$acc_identity_report, correct = FALSE)
```
...
The reproducibility check for this paper was unsuccessfully. The values of task performance were reproduced accurately, but the Chi Squared test values could not be reproduced. I compared the surprise and first control trial as specified in the manuscript, but was unable to reproduce the two test statistics that were originally reported.
