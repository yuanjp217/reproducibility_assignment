...
#dataset consisting of just the accuracy
acc_d <- d%>%
...
```{r 89% presurprise}
m = count(acc_d)
presurprise_acc = sum(acc_d$acc_location_report) / m
presurprise_acc
```
```{r}
surprise_data <-acc_d %>%
...
```{r surprise performance}
#below is code to list how many subjects correctly did the task divided by the n=20 for percentage.
#these are the subjects who correctly did the color task
correct_color <- surprise_data %>%
  filter(acc_color_report == 1)
length(correct_color$id)

#these are the subjects who correctly did the identity task
correct_identity <- surprise_data %>%
  filter(acc_identity_report == 1)
length(correct_identity$id)

# surprise - location task
correct_location <- surprise_data %>%
  filter(acc_location_report == 1)
length(correct_location$id)
```

```{r control performance}
ctrl = acc_d %>%
  filter(trial == 157)
#1st control trial's letter color task
sum(ctrl$acc_color_report == 1) / 20
#1st control trial's letter identity task
sum(ctrl$acc_identity_report == 1) / 20
...
head(chi_set)
#chisq.test(ctrl$acc_color_report,surprise_data$acc_color_report)
chisq.test(chi_set$acc_color_report,NULL)
```
